...
data testbed.housing; set simhpi.housing; indexcode=put(cbsa_code,$5.); keep indexcode housing; run;
...
proc means data=allsim noprint; class simid qtr; weight housing; var unemp inc_p50 rentidx; output out=US mean= sumwgt=housing;run;
proc means data=US noprint; class qtr;  var unemp inc_p50 rentidx; output out=US_mean mean= ; run;
...
proc means data= allsim_state noprint nway; class state simid qtr; var unemp inc_p50 rentidx; weight weight; output out=state mean=; run;
proc means data=state noprint nway; class state qtr; var unemp inc_p50 rentidx ; output out=state_mean mean=; run;

data allsim_output; set allsim us(in=f1) state(in=f2);
if f1 then indexcode='US'; 
if f2 then  indexcode=state;
if  indexcode ne '' and qtr>0;
drop housing _TYPE_ _FREQ_ state;
run;

...
proc univariate data=geoW noprint; ; var pInst p_sfr p_rentocc;  output out=P_momPop pctlpre=Inst_  sfr_ rentocc_ pctlpts= 33,40,50,67, 70; run;
proc univariate data=geoW noprint;class msrMarket ; var pInst p_sfr p_rentocc;  output out=P_momPop2 pctlpre=Inst_  sfr_ rentocc_ pctlpts= 33,40,50,67, 70; run;

proc sql; create table geoW as
select a.*
, case when p_SFR<=p.sfr_33 then 1 when p_sfr<=p.sfr_67 then 2 else 3 end as pSFR_group
, case when A.msrMarket=1 then
  ...
  END 
as pSFR_group2
, case when A.msrMarket=1 then 1
	
  ELSE 
  	case when p_SFR<=p2.sfr_33 then 2 when p_SFR<=p2.sfr_67 then 3 else 4 end 
  END
as pSFR_group3
, case when pInst<=p.Inst_33 then 1 when pInst<=p.Inst_67 then 2 else 3 end as Inst_group
, case when pInst<=p2.Inst_33 then 1 when pInst<=p2.Inst_67 then 2   else 3 end as Inst_group2
, case when p_rentocc<=p.rentocc_33 then 1 when p_rentOcc<=p.rentocc_67 then 2 else 3 end as rentocc_group
, case when p_rentocc<=p2.rentocc_33 then 1 when p_rentOcc<=p2.rentocc_67 then 2  else 3 end as rentocc_group2
from geoW a
join p_momPop p
on 1=1
join P_momPop2 p2
...
, pSFR_group, inst_group, rentocc_group, pSFR_group2, inst_group2,pSFR_group3
...
if taxRate>0.08 or taxRate<0.005 then taxRate=.;
if GeoCode^=. && Year^=.; 
if geoCode>0;
...
run;
proc univariate data=modelinp1(where=(qtr>=201001 and qtr<=202001)); var rent2own hpg_season_last2; histogram; run;
...
data modelinp2a; set modelinp2;  by indexcode indexmonth;
...
tp = (medHP_l1*factor) * m_rent2Own;
tpg= tp/lag(tp);
if indexcode ne lag(indexcode) then tpg=.;
if not first.indexcode; run;

data modelinp2a; set modelinp2a;  by indexcode indexmonth;
if first.indexcode then tpg=1;
retain fundrent;
if first.indexcode then fundRent=1;
fundRent = fundRent*tpg;
*fundRent=log(FundRent);
ln_month = log(month);
 run;
proc univariate data=modelinp2(where=(qtr>=201001 and qtr<=202103)); var afford_rent2; histogram; run;
 proc  means ; class pSFR_group3;*inst_group; var hpg_season_last2 afford_rent2; run;
...
select distinct a.*, b.msrMarket, b.pSFR_group, b.pSFR_group2, h.m_mortshare, h.m_rentShare, h.m_rent2own
...
** Try group by pSFR_group/pSFR_Group2 again after remove outlier;

data modelinp3; set modelinp2a; where indexcode not in (&outlier1.); 
drop tp tpg;
fundRent = log(fundRent);
/*
if msrMarket=1 then pSFR_group3=0;
else pSFR_group3=pSFR_group2;
*/
run; proc means data=modelinp3; class pSFR_Group;vaR hpg_season_last2; run;

proc export data=modelinp3 outfile="E:\Output\Rent Forecast\forecast input.csv" dbms=csv replace; run;

** Try parameter with constraints;

%let version=1; %let geoW=geoW;%let fundV =  month ln_inc afford_rent rent2own inc_gini; %let byV= pSFR_group ; %let stage3V=afford_HP;

data parmmap;
length parameter $2. parm $15. initial $1. bound $3.;
...
;
run;
%let name=parm_stage3; %let var=&byv.; %let maxGroup=&maxgroup.; %let inp =is_stage2_v&version.; %let yvar= r_stage2;
%let i=1;
...
%let version=3; %let byV=psfr_group3; %let maxgroup=4;
** Fundamental Line;
%macro tryv_wConstraint(version=, byV=, maxgroup=);
proc sort data=modelinp3; by  &byV. indexcode qtr; run;

proc nlin data=modelinp3(where=(qtr>201100 and qtr<=202103 )) /*outest=parm_fund_v&version. */  ;  by &byV.; 
   parameters b0=0 b1=0 b2=0 b3=0 b4=0 b5=0;
   bounds b1 >= 0, b2>=0, b3>=0, b4>=0, b5<=0;
   model ln_rentIdx = b0 + b1*ln_month + b2* ln_inc +b3*inc_gini + b4*afford_rent + b5*rent2Own;
   ods select ParameterEstimates;
   ods output  ParameterEstimates=parmFund_v&version.; *AdditionalEstimates;
run;

data parmFund_v&version.; set parmFund_v&version.(where=(label = '')); drop label;
if probt>0.05 AND parameter ne 'b0' then do; 
estimate=0; stderr=. ;lowerCL=.; upperCL=.; tValue=.;
end;
run;
proc sort data=parmfund_v&version.; by parameter;
proc sort data=parmmap; by parameter;
data parmfund_v&version.; merge parmfund_v&version.(in=f1) parmmap(in=f2); by parameter;if f1; run;
proc sort; by &byv.; run;

data FinalparmFund_v&Version.;  run;
%loop(name=parmFund,var=&byv., maxGroup=&maxgroup.,  inp =modelinp3, yvar= ln_rentidx) ;
proc transpose data=FinalparmFund_v&version. out=parmfund; by &byv.; var estimate; id parameter; run;
...
proc reg data=is_fund_v&version.(where=(qtr>201100 and qtr<=202103)) outest=parm_stage2_v&version. adjrsq tableout noprint;  by &byv.;*weight housing; 
weight &geoW.;*where qtr>=201101 and  qtr<202103;
model lnRentg = rhpits  rentg_l1 rentg_l2 rentg_l3 rentg_l4 rentg_l5 inc_gini ln_rent2own afford_rent /selection=stepwise sle=0.05 /*sle=0.001 sle=0.05*/;
output out=is_stage2_v&version. r=r_stage2 p=p_stage2; run; quit;

** 3rd stage: HPA and other Var;

** Constrained;

proc nlin data=is_stage2_v&version. /*outest=parm_fund_v&version. */  ;  by &byV.; 
   parameters b0=0 b1=0 b2=0 b3=0 b4=0 b5=0;
   bounds b1 >= 0, b2<=0, b3<=0, b4>=0;
   model r_stage2 = b0 + b1* hpg_season_last2 + b2* unemp_g + b3* rentyield_l1 + b4* chgslope0_1 + b5* afford_HP;
   ods select ParameterEstimates;
   ods output  ParameterEstimates=parm_stage3_v&version.; *AdditionalEstimates;
run;

data parm_stage3_v&version.; set parm_stage3_v&version.(where=(label = '')); drop label;
if probt>0.05 and parameter ne 'b0' then do; 
estimate=0; stderr=. ;lowerCL=.; upperCL=.; tValue=.;
end;
run;
proc sort data=parm_stage3_v&version.; by parameter;
proc sort data=parmmap2; by parameter;
data parm_stage3_v&version.; merge parm_stage3_v&version.(in=f1) parmmap2(in=f2); by parameter;if f1; run;
proc sort; by &byv.; run;

data Finalparm_stage3_v&Version.;  run;
%loop(name=parm_stage3,var=&byv., maxGroup=&maxgroup.,  inp =is_stage2_v&version., yvar= r_stage2) ;

** Total R-square;
proc transpose data=Finalparm_stage3_v&version. out=parm_stage3; var estimate; id parameter; by &byv.;
run;

data parm_stage3; set parm_stage3;
if b0=. then b0=0;
if b1=. then b1=0;
if b2=. then b2=0;
if b3=. then b3=0;
...
data resid_SFrent_v&version.; merge is_stage2_v&version.(in=f1) parm_stage3(in=f2 drop=_name_); by &byv.; if f1 and f2;
p_rentg = p_stage2 +  b0 + b1* hpg_season_last2 + b2* unemp_g + b3* rentyield_l1 + b4* chgslope0_1 + b5* afford_HP;
...
** Summarize;
proc sql; create table finalParam_fund as
select distinct b.parm as var, a.&byv., a.estimate as PARMS, a.stderr as STDERR, a.tValue as T, a.probt as PVALUE
from finalparmfund_v&version. a
join parmmap b
on a.parameter=b.parameter
order by &byv.;
quit;

proc transpose data=finalparam_fund out=finalparam_fund; var PARMS stderr t pvalue; by &byv.; id var; run;

data finalParam_fund; retain &byv. _name_ intercept ln_month ln_inc inc_gini rent2own afford_rent; set finalParam_fund;
drop _label_;
rename _name_=_type_ intercept= fund_int;
run;
data finalParam_fund; merge finalParam_fund(in=f1) fundr_sum(in=f2 keep=&byv. r2 rename=(r2=fund_r2)); by &byv.;run;



proc sql; create table finalparam_stage3 as
select distinct b.parm as var, a.&byv., a.estimate as PARMS, a.stderr as STDERR, a.tValue as T, a.probt as PVALUE
from finalparm_stage3_v&version. a
join parmmap2 b
on a.parameter=b.parameter
order by &byv.;
quit;

proc transpose data=finalparam_stage3 out=finalparam_stage3; var PARMS stderr t pvalue; by &byv.; id var; run;

data finalparam_stage3; retain &byv. _name_ intercept hpg_season_last2 unemp_g rentyield_l1 chgslope0_1 afford_HP; set finalparam_stage3;
drop _label_;
rename _name_=_type_ intercept=stage3_int ;
run;

proc sort data=finalParam_fund; by &byv. _type_;
proc sort data= parm_stage2_v&version.; by &byv. _type_;
proc sort data= finalparam_stage3; by &byv. _type_;

data finalParam_v&version.; merge finalParam_fund(in=f1) parm_stage2_v&version.(in=f2 where=(_type_ in('PARMS','STDERR','T','PVALUE')) 
rename=(inc_gini= inc_gini_stage2 ln_rent2Own = ln_rent2Own_stage2 afford_Rent = afford_rent_stage2)
drop=_DEPVAR_ _RMSE_ _MODEL_ lnrentg _in_ _p_ _EDF_ _RSQ_ _adjRSQ_) finalparam_stage3; by &byv. _type_; run;
data finalParam_v&version.; merge  finalParam_v&version.(in=f1) ttr_sum(in=f2 keep=&byv. r2 rename=(r2=total_r2)); by &byv.; run;
%mend;

* V1: by SFRgroup only;
%tryv_wConstraint(version=1, byV=psfr_group, maxgroup=3);
* V2: MSR Market & SFR Group;
%tryv_wConstraint(version=2, byV=psfr_group2, maxgroup=6);
* V2: MSR Market + SFR Group;
%tryv_wConstraint(version=3, byV=psfr_group3, maxgroup=4);

data summary; set finalParam_v1(in=f1) finalParam_v2(in=f2) finalParam_v3(in=f3);
if f1 then do; version=1; indexcode=psfr_group; end;
if f2 then do; version=2;indexcode=psfr_group2; end;
if f3 then do; version=3;indexcode=psfr_group3; end;
drop psfr_group psfr_group2 psfr_group3;
run;

proc export data=summary outfile="E:\Output\Rent Forecast\tried version summary 1209 v3.csv" dbms=csv replace; run;

/*
proc reg data=is_stage2_v&version.(where=(qtr>201100 and qtr<=202103)) outest=parm_stage3_v&version. adjrsq tableout noprint;   by &byv.; *by indexcode;   
weight &geoW.;
model r_stage2= hpg_season_last2 unemp_g rentyield_l1 chgslope0_1 afford_HP /selection=stepwise sle=0.05;*hpg_season unemp_g  hpg_season_l1 rentyield_l1  chgslope0_1;
; output out=resid_SFrent_v&version. r=r_SFRent p=P_rentg; run; quit; 
*/

** find market that fundamental line does not work;
proc sort data=modelinp2; by indexcode;
proc reg data=modelinp2(where=(qtr>201100 and qtr<=202103 and indexcode not in (&outlier1.))) outest=parm_fund_outlier adjrsq noprint tableout ;
;*by indexcode; *by MSRmarket;  weight geoW;  *where qtr>201100 and qtr<202001;
model ln_Rentidx = inc_gini month afford_rent rent2own ln_inc  ln_sfdhh /selection=stepwise sle=0.001 ;* ln_SFR ln_laborforce  ln_laborForce ln_emp afford_HP;
output out=fund_outlier r=r_fund p=rentidx_fund; run; quit;

** 2nd Stage: AR(5);
proc sort data=fund_outlier; by indexcode qtr; run;
data fund_outlier; set fund_outlier(where=(rentidx_fund ne .)) ; *intercept=1; by indexcode qtr;
l1_fundidx=lag(rentidx_fund);
if first.indexcode then l1_fundidx=.;
rhpits = ln_rentidx_l1-l1_fundidx;
rhpits_l1 = lag(rhpits);
...
proc reg data=fund_outlier(where=(qtr>201100 and qtr<=202103)) outest=fund_outlier_2 adjrsq tableout noprint; by indexcode;*weight housing; 
model lnRentg = rhpits  rentg_l1 rentg_l2 rentg_l3 rentg_l4 rentg_l5 /selection=stepwise sle=0.1 /*sle=0.001 sle=0.05*/;
; run; quit;

 /*
proc sort data=modelinp2; by msrmarket; run;
...
PROC REG data=fc_outlier (where=(qtr>201100 and qtr<=202103)) outest=reg_act_outlier adjrsq tableout noprint;  by indexcode;   
model lnRentg = model1
;  run; quit; 

data reg_act_outlier; set reg_act_outlier; msrmarket=0; if indexcode in (&amherstMarket.) then msrMarket=1;run;
proc means data=reg_act_outlier(where=(_type_='PARMS')) p1 p5 p10 p90 p95 p99; VAR model1; class msrMarket; RUN;
...
*6.   You can also throw in ln(household) & income inequality to try...;
%tryModel(version=8, geoW=geoW, byMarket=by pSFR_group2, fundV =  month ln_inc afford_rent rent2own inc_gini , byV= pSFR_group2, stage3V=afford_HP );
%tryModel(version=8A, geoW=geoW, WHERE= and MSRMarket=1, byMarket=by MSRMarket, fundV =ln_inc afford_HP month rent2own, byV= MSRMarket, stage3V= );



data summary2;retain idea ver msrMarket pSFR_group2 _type_ fund_int month ln_inc  afford_rent rent2Own afford_HP 
fund_r2 fund_adjr2 stage2_int  rhpits  rentg_l1 rentg_l2 rentg_l3 rentg_l4 rentg_l5 sum_ar stage2_r2 stage2_adjr2
stage3_int hpg_season unemp_g  hpg_season_l1-hpg_season_l4 rentyield_l1  chgslope0_1  stage3_r2 stage3_adjr2 totalR2
; set parm_final_v8:;
length idea ver $5.;
...
if psfr_group2=1  then delete;
if _type_ ne 'PARMS' then totalR2=.;
if pSFR_group2 ne . then pSFR_group2 = psfr_group2-1;
run;
proc export data=summary2 outfile="E:\Output\Rent Forecast\tried version summary new.csv" dbms=csv replace; run;

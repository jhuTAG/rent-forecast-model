...
data infBE; *https://www.imf.org/external/datamapper/PCPIEPCH@WEO/OEMDC/ADVEC/WEOWORLD/USA;
input yr annualInf;
datalines;
2022	5.0338
2023	2.9849
...
data infBE; set infBE; asofDate=202206;baseyr=int(asofdate/100); run;

data infMonthlyBE; set infBE; do year=yr to yr+1; do monthID=1 to 12; infBE=.;
 if (monthID>mod(asofDate,100) and year=yr) or (monthID<=mod(asofDate,100) and year=yr+1) 
then infBE=(annualInf/100+1)**(1/12)-1; month=year*100+monthID; output; end;  end;
...
from infFC left join YTDCPI
on infFC.yr=int(YTDCPI.month/100)
join monthID on 1=1
join YTDCPI ytdmo on yr*100+monthID>ytdcpi.month ; proc sort nodup;  by  month;run;
...
data cpiqtr_withFC; set cpiqtr_withFC; by qtr; inflation=(CPI)/lag(CPI)-1; inflation_BE=(cpi_BE)/lag(cpi_BE)-1; if cpi ne .; run;
...
** GET ELASTICITY;
data landelastic;infile "\\tvodevw10\T$\Thu Output\HPI\HPI Calculation\v3.0\SAS Input\Long term HPI inputs\top100cbsa info_with undevelopable.txt" delimiter = '09'x missover dsd lrecl=32767 firstobs=1 ; format moodyname $50.;
...
insurerate=0.5; 
indexcode=put(cbsa_div,$5.); keep indexcode landelstic; 
...
data modelinp1; merge modelinp0b(in=f1) ipums5; by indexcode  indexmonth; if f1; retain ln_LtLine Rentidx1 t ;  
...
proc means data=modelinp1 noprint; weight housing; 
var  incg LnRentg hpg_season rentyield_l1 rentyield RTI RTI_l1 Rent2Own Rent2Own_l1 Rent2Own_l2 Rent2Own_l3 Rent2Own_l4 Rent2Own_l6 Rent2Own_l8 PITI_nolog
...
keep indexmonth date US_ProdHPI; run; proc sort nodup; by date; run;
...
data irs.rentforecast_parm;  set allparm;
*/
...
data qtr; merge qtr(in=f1) SimHPI(in=f2 keep=simid m_ln_hpi_season hpg_season unemp qtr indexcode unemp SFDHousehold  hpg_season_l1
...
retain rentidx HPI cpi;
if first.indexcode then do; rentidx=1; hpi=1; CPI=1; end;
else do; rentidx=rentidx*(1+rentg); hpi=hpi*(1+hpg);CPI=CPI*(1+inflation); end;
keep qtr  indexcode simid  vacancy pprcaprate rentidx hpi fundspread  unemp SFDHousehold Inc_p50 Inc_mean refi_rate
cmt_2yr cmt_10yr libor_3m  capr_ust10y_g_l1 capr_ust10y_g capr_ust10y cmt_10yr pprcaprate sp500_chg sp500_idx CPI;
run;
data allSim; set allSim fc5; if simid ne .; run;
%mend;

data ALlSim;run;
%loopSim(startsim=1, endsim=500, infShock=0, HPAShock=0);
...
proc means data=allsim noprint; class   simid qtr; weight housing; var vacancy --hpi cmt_10yr sp500_chg sp500_idx CPI; output out=US mean= sumwgt=housing;run;
proc means data=US noprint; class    qtr;  var vacancy --hpi housing sp500_chg sp500_idx  cmt_10yr CPI; output out=US_mean mean= ; run;
...
%put &enddate.;
proc delete data=testbed.bak_RentHPIMeanPath_v2022_&enddate.;
data testbed.bak_RentHPIMeanPath_v2022_&enddate.(insertbuff=32000); set irs.RentHPIMeanPath_monthly_v2022; *_FIXEDHPA;run;

proc delete data=irs.RentHPIMeanPath_monthly_v2022; *_FIXEDHPA;
data irs.RentHPIMeanPath_monthly_v2022(insertbuff=30000); set MeanPath ;  if indexcode ne ''  and date>0; 
RateasofDate=20210305; drop simid _TYPE_ _FREQ_ capr_ust10y_g_l1 capr_ust10y_g_l1  capr_ust10y; run;

proc delete data=testbed.bak_SimRentHpi_v2022_&enddate.;
data  testbed.bak_SimRentHpi_monthly_&enddate.(insertbuff=32000); set irs.SimRentHpi_monthly_v2022;run;


proc delete data=irs.SimRentHpi_monthly_v2022;
data irs.SimRentHpi_monthly_v2022(insertbuff=30000); set allsim_output_Monthly; run;

proc delete data=irs.HistMFcaprateVac_v2022;
data irs.HistMFcaprateVac_v2022(insertbuff=30000); set pprCBSA; run;
...
proc delete data=testbed.bak_sf_rentIdx_month_dt_v2022;
data  testbed.bak_sf_rentIdx_month_dt_v2022(insertbuff=30000); set testbed.load_sf_rentIdx_month_dt_v2022; ; run;

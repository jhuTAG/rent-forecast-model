...
, case when a.MSRmarket=1 then 1 when p_SFR<=p2.sfr_33 then 2 when p_SFR<=p2.sfr_67 then 3 else 4 end as pSFR_group2
, case when pInst<=p.Inst_33 then 1 when pInst<=p.Inst_67 then 2 else 3 end as Inst_group
, case when a.MSRmarket=1 then 1 when pInst<=p2.Inst_33 then 2 when pInst<=p2.Inst_67 then 3   else 4 end as Inst_group2
, case when p_rentocc<=p.rentocc_33 then 1 when p_rentOcc<=p.rentocc_67 then 2 else 3 end as rentocc_group
, case when a.MSRmarket=1 then 1 when p_rentocc<=p2.rentocc_33 then 2 when p_rentOcc<=p2.rentocc_67 then 3  else 4 end as rentocc_group2
from geoW a
join p_momPop p
on 1=1
join P_momPop2 p2

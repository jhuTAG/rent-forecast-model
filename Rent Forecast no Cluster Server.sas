...
data errmat_sp2; set irs.errmat_sim_sp ;*errmat_sp1;by simid qtridx; retain qtr; if first.simid then qtr=&fcqtrStart; 
else do; 
...
data errMat2; set irs.errmat_sim_rentfc/*errMat_RentFC*/(where=(&startsim<=simid<=&endsim));by indexcode simid qtridx; retain qtr; if first.simid then qtr=&fcqtrStart; 
else do; 
if mod(qtr,100)=4 then qtr=qtr+100-3; else qtr+1;
end; drop qtridx; run;
...
/*
%let lt_out=\\tvodevw10.CORP.amherst.com\T$\Thu Output\HPI\HPI Forecast\v2.1;

libname SimHPI "&lt_out.\parameters"; 
proc delete data=irs.hpi_ln_seasonality; 
data irs.hpi_ln_seasonality; set simhpi.ln_seasonality; run;

*/

data hpiseasonality; set  irs.hpi_ln_seasonality(rename=(qtr=qtridx seasonality=seasonalityHPI));
...
libname myresult "\\tvodevw10\T$\Thu Output\HPI\HPI Calculation\v2.0\evenmonth";

data irs.CBSAStateAVM; set myresult.CBSAStateAVM;
data irs.CBSAStateSFD; set myresult.CBSAStateSFD;run;
*/

data CBSAstate; merge irs.CBSAStateAVM(in=f1) irs.CBSAStateSFD(in=f2); by state cbsa_div; if f1 and f2;
...
/*
proc delete data=testbed.bak_sf_rentIdx_month_dt_sql&enddate.;

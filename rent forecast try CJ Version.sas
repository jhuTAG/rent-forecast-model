...
LIBNAME irs ODBC DSN='irsPublish' schema=dbo;
LIBNAME testbed ODBC DSN='Apollo_TestBed' schema=dbo;
LIBNAME thirdp ODBC DSN='Apollo_3Party' schema=dbo;
LIBNAME ahpi ODBC DSN='Xu_HPI' schema=dbo;
**%let lt_out=&tDrive.\Thu Output\HPI\HPI Forecast\v2.1;

**LIBNAME Parm "\\tvodev\T$\Thu Output\HPI\HPI Forecast\v2.1\parameters";
%let amherstmarket = ('12060', '16740', '28140', '45300', '38060', '36740', 
'34980', '27260', '23104', '31140', '19124', '32820', '13820', '36420', '26900', 
'39580', '41180', '41700', '29460', '15980', '35840', '49180', '26420', '37340', 
'46060', '24660', '19660', '18140', '29820', '19740', '48424', '22744', '20500', 
'38940', '45104', '33124', '15500','41620','42644');
...
proc SQL; connect to odbc(DSN='Apollo_TestBed'); 
create table allRent00 as select * 
...
proc sql; connect to odbc(DSN='Apollo_3Party');
create table ACS as select distinct * from connection to odbc(
  select distinct a.GeographyType, case when a.geographytype='Nation' then 'US' else isnull(cast(s.state as varchar(5)), a.GeographyCode) end 
as indexcode,year(a.begindate) as year, a.Value as SFD, b.value as SFA, c.value + isnull(d.value,0) as SFR
, isnull(oc.value,0)+isnull(oc1.value,0) as SF_OwnerOcc, e.value as RenterOcc, tc.value as total_Occ, m.value as MedRent_ACS

from ThirdPartyData..DemoEcon_dt a left join ThirdPartyData..DemoEcon_dt b
...
on cast(a.GeographyCode as bigint)=cast(c.GeographyCode as bigint) and c.GeographyType=a.GeographyType
and a.BeginDate=c.BeginDate and c.DataSeries='Renter-Occupied Housing Units, 1, Detached Unit, 1-Year Estimate' 
left join ThirdPartyData..DemoEcon_dt d
on cast(a.GeographyCode as bigint)=cast(d.GeographyCode as bigint) and d.GeographyType=a.GeographyType
and a.BeginDate=d.BeginDate and d.DataSeries='Renter-Occupied Housing Units, 1, Attached Unit, 1-Year Estimate' 
left join ThirdPartyData..DemoEcon_dt e
on cast(a.GeographyCode as bigint)=cast(e.GeographyCode as bigint) and a.BeginDate=e.BeginDate 
and e.DataSeries='Renter-Occupied Housing Units, 1-Year Estimate' and e.GeographyType=a.GeographyType
left join ThirdPartyData..DemoEcon_dt m
on cast(a.GeographyCode as bigint)=cast(m.GeographyCode as bigint) and a.BeginDate=m.BeginDate 
and m.DataSeries='Median Gross Rent, 1-Year Estimate' and m.GeographyType=a.GeographyType

left join ThirdPartyData..DemoEcon_dt tc
on cast(a.GeographyCode as bigint)=cast(tc.GeographyCode as bigint) and a.BeginDate=tc.BeginDate 
and tc.DataSeries='Total Occupied Housing Units, 1-Year Estimate' and tc.GeographyType=a.GeographyType

left join ThirdPartyData..DemoEcon_dt oc
on cast(a.GeographyCode as bigint)=cast(oc.GeographyCode as bigint) and a.BeginDate=oc.BeginDate 
and oc.DataSeries='Owner-Occupied Housing Units, 1, Detached Unit, 1-Year Estimate' and oc.GeographyType=a.GeographyType
left join ThirdPartyData..DemoEcon_dt oc1
on cast(a.GeographyCode as bigint)=cast(oc1.GeographyCode as bigint) and a.BeginDate=oc1.BeginDate
and oc1.DataSeries='Owner-Occupied Housing Units, 1, Attached Unit, 1-Year Estimate' and oc1.GeographyType=a.GeographyType

left join ThirdPartyData..State_dt s on cast(a.GeographyCode as bigint)=cast(s.StateFIPS as bigint)

where a.DataSeries='Housing Units, 1 Unit, Detached, 1-Year Estimate' and a.GeographyType not in ('Census Tract')
order by a.GeographyType,indexcode, year
); disconnect from odbc; quit; proc sort data=ACS nodup; by indexcode year;run;

data rentunits; set ACS; by indexcode year; SF=sfd+sfa; if sfr=. then sfr=0;
if renterOcc=. then renterocc=0; if total_occ=. then total_occ=0;
if sfr+sf_ownerOcc>0 then p_sfr = sfr/(sfr+sf_ownerOcc); **% rentals among SF units;
if total_occ>0 then p_rentocc = renterocc/total_occ; **% rentals among all units;
if indexcode='16974' then indexcode='16984'; if indexcode='19380' then indexcode='19430';
if indexcode='43524' then indexcode='23224';	run;  proc sort; by indexcode year;run;


data rentUnits_rescale; set rentunits(where=(geographytype in ('CBSA','Metro Division'))); 
by indexcode year; SFR_g = sfr/lag(SFR);  if first.indexcode then do; sfr_g=.; end; 
/*sfr_g_l1 = lag(sfr_g); if indexcode ne lag2(indexcode) then sfr_g_l1=.;
sfr_g_l2 = lag(sfr_g_l1); if indexcode ne lag3(indexcode) then sfr_g_l2=.;		*/	run;
...
(
  select isnull(t.cbsa_div, t.cbsa) as indexcode, count(1) as nLease,
	1-sum(case when owner='Individual' then 1 else 0 end )*1.0/count(1) as pInst

  from ModelTestBed.dbo.SFR_Rent_CleanUp_new_final a
  join amhersthpi..hpi_taxroll_vw t   on a.asg_propid= t.asg_propid and t.prop_type='SF'
  where isnull(t.cbsa_div, t.cbsa) is not null and year(lease_enddate)>=2016  and lease_enddate<'2021-07-01'
  group by isnull(t.cbsa_div, t.cbsa)   having count(1)>250
  order by isnull(t.cbsa_div, t.cbsa) 	
); disconnect from odbc; quit;


proc sql  noprint; create table Listing2013 as 
select distinct cbsa,count( asg_propid) as closedListing, median(closingRent) as medPrice,
avg(closingRent) as avgPrice from repeatedSale where year(lease_enddate)=2013 and cbsa ne ''
group by cbsa order by cbsa; quit;

proc sql  noprint; create table Listing2016 as 
select distinct cbsa,count( asg_propid) as closedListing,median(closingRent) as medPrice,
avg(closingRent) as avgPrice from repeatedSale where year(lease_enddate)=2016 and cbsa ne ''
group by cbsa order by cbsa; quit;


data geoW; merge listing2013(in=f1) rentunits(in=f2 rename=indexcode=cbsa where=(year=2013)) 
momPop_share(rename=(indexcode=cbsa)) ;  by cbsa;  if f1 and f2 ; if cbsa ne ''; 
geoW=SFR*avgPrice; geoW_allRentUnit = RenterOcc*medRent_ACS; p_sfr_allrent = sfr/renterocc;
...
select distinct c.Name as Market,  case when a.cbsa in  &amherstmarket. then 1 else 0 end as MSRMarket
,  a.* from geoW a left join thirdp.cbsa_dt c on a.cbsa = c.cbsa
where a.cbsa in (select distinct indexcode from irs.sf_rentidx_monthly) order by cbsa; quit;
...
proc sql; create table geoW as 
select a.*, case when p_SFR<=p.sfr_40 then 1 when p_sfr<=p.sfr_70 then 2 else 3 end as pSFR_group
...
from geoW a join p_momPop p on 1=1 join P_momPop2 p2 on p2.msrMarket=a.msrMarket order by cbsa;  quit;


data geoW_2016; merge listing2016(in=f1) rentunits(in=f2 rename=indexcode=cbsa where=(year=2016))
momPop_share(rename=(indexcode=cbsa)) ;  by cbsa;  if f1 and f2 ; if cbsa ne ''; 
geoW=SFR*avgPrice; geoW_allRentUnit = RenterOcc*medRent_ACS; p_sfr_allrent = sfr/renterocc;
keep cbsa  geoW SFR RenterOcc SFD MedRent_ACS SF p_SFR geoW_allRentUnit p_rentocc p_sfr_allrent nlease pInst avgPrice;
run;
...
select distinct c.Name as Market,  case when a.cbsa in &amherstmarket. then 1 else 0 end as MSRMarket
,  a.* from geoW_2016 a left join thirdp.cbsa_dt c on a.cbsa = c.cbsa
where a.cbsa in (select distinct indexcode from irs.sf_rentidx_monthly) order by cbsa; quit;
...
proc sql; create table geoW_2016 as select a.*
...
from geoW_2016 a join p_momPop2016 p on 1=1 order by cbsa; quit;

proc export data=geoW outfile="D:\Model Tracking\Repeated Rent Indices\weight by cbsa.csv" dbms=csv replace; run;
proc export data=geoW_2016 outfile="D:\Model Tracking\Repeated Rent Indices\weight by cbsa 2016.csv" dbms=csv replace; run;


data rawRentidxOrg; set irs.sf_rentidx_monthly ; date=year(monthfmt)*100+month(monthfmt); *if pricetier='agg'; 
*substr(monthfmt,1,4)*100+substr(monthfmt,6,2); drop monthfmt; *drop index_: cbsa;*  if city20=0 or city20=.;
where indexcode not in ('90000','91000'); keep indexcode index date; proc sort nodup; by indexcode date; run;
...
**what does this part do? To assume next 1- or 2-month rentg is the same as this month if this month is not quarter-end;
...
data rawrentIdx; merge rawRentidxOrg(rename=index=rentidx0) rawrentIdx3; by indexcode date; 
retain index; if first.indexcode then index=.; if rentidx0>0 then index=rentidx0;
else index=index*(1+rentg); drop rentidx0 rentg; if mod(date,100)  in (3,6,9,12); run;

/* **just to be sure the coding above does not cause problems; 
data cj; merge rawRentidxOrg(rename=index=rentidx0) rawrentIdx3; by indexcode date; 
data cj; set cj; by indexcode date; retain cj_index; if first.indexcode then cj_index=.;
if cj_index=. && rentidx0>0 then cj_index=rentidx0; else cj_index=cj_index*(1+rentg);
drop rentidx0 rentg; if mod(date,100)  in (3,6,9,12); run;
data compare; merge rawRentIdx(in=f0) cj(in=f1); by indexCode date; if f0 && f1; proc means; var index cj_index; run; 
*/


data allrentIdx; set rawrentIdx; *drop geographytype; *if length(geographycode)=5; 
indexmonth=input(put(date*100+1,8.),YYMMDD10.); format indexmonth  MMDDYYD10.; 
rename date=qtr index=rentidx; keep indexcode date index indexmonth; run;
proc sql; create table allrentIdx as select distinct * from allrentIdx group by indexcode 
having min(qtr)<201001 and max(qtr)>=201901 order by indexcode ,qtr;quit;
proc x12 data=allrentIdx date=indexmonth noprint interval=QTR; by indexcode ; 
var Rentidx;  x11;  output out=sa d11;   ods select d11; run;

data allrentIdx; merge  allrentIdx(in=f1) sa(in=f2 rename=Rentidx_D11=Rentidx_sa); 
by indexcode  indexmonth;  if f1 and f2; LnRentg=log(Rentidx_sa/lag(Rentidx_sa)); 
if first.indexcode then do; LnRentg=.; end; qtr=year(indexmonth)*100+month(indexmonth); 
if lnrentg=. then delete; qtr=int(qtr/100)*100+mod(qtr,100)/3;run; 

data tp; set allrentidx; by indexcode indexmonth; YOYrentg = log(rentidx/lag4(rentidx));
if indexcode ne lag4(indexcode) then YOYRentg=.; if mod(qtr,100)=4; year = int(qtr/100); run;


data _null_; set thirdp.county_dt;if cbsadiv='' then cbsadiv=cbsa;
call symput(compress("fipscbsa"||trim(fips)), cbsadiv); run; %put &fipscbsa01001;

proc sql; connect to odbc(DSN='Apollo_3Party');
create table Cntyemp as select distinct * from connection to odbc
(
select  geographycode = substring(area_code, 3, 5), month=year*100+right(period,2), value as totemp
 from thirdpartydata..bls_la_current where series_title like '%Employment%' and measure_name='employment'
	and area_type_name = 'Counties and equivalents' and seasonal_name='Not Seasonally Adjusted' 
	and period <> 'M13' order by geographycode, month
);disconnect from odbc; quit;

proc sql; connect to odbc(DSN='Apollo_3Party');
create table CntyLabor as select distinct * from connection to odbc
(
select  geographycode = substring(area_code, 3, 5), month=year*100+right(period,2), value as laborforce
 from thirdpartydata..bls_la_current where series_title like '%Labor Force%'
	and area_type_name = 'Counties and equivalents' and seasonal_name='Not Seasonally Adjusted'
	and period <> 'M13' and value>0  order by geographycode, month
);disconnect from odbc; quit;
proc sort data=cntylabor; by geographycode month; proc sort data=Cntyemp; by geographycode month; run;


data cntyUnemp0; merge cntyEmp(in=f1) cntyLabor(in=f2); by geographycode month; if f1 and f2;
if mod(month,100)<=3 then tp=1; else if mod(month,100)<=6 then tp=2; else if mod(month,100)<=9 then tp=3; 
else tp=4;  qtr=input(put(int(month/100)*10000+tp*300+1,8.),yymmdd10.); format qtr  monyy.; run;
proc means data=cntyUnemp0 noprint nway; class geographycode qtr; var totemp laborforce; output out=countyemp mean=;run;

data countyemp; set countyemp; where geographycode ne '' and qtr ne .; cbsa_code=
symget(compress("fipscbsa"||geographycode)); keep geographycode qtr cbsa_code totemp laborforce;; run;  
...
unemp=1-totemp/laborforce; ln_emp = log(totemp); ln_laborforce = log(laborforce); 
keep ln_emp ln_laborforce totemp laborforce cbsa_code qtr unemp; run; **unemp was commented out, so the code failed;
...
proc sql; create table sfr as select a.* from rentUnits_rescale a where a.indexcode 
in (select distinct indexcode from allrentidx) order by indexcode, year; quit;
...
from year b join sfr_last a on 1=1
join sfr_last a1 on a.indexcode=a1.indexcode and b.year>=a1.year 
order by indexcode, year; quit;

data sfr_1; set sfr_1; by indexcode year; retain sfr;
if first.indexcode then sfr=sfr0; else sfr = sfr*sfr_g; run;

data sfrUnit; set sfr(keep=indexcode year sfr) sfr_1(keep=indexcode year sfr); 
ln_SFR = log(sfr);  proc sort nodup; by indexcode year; run;

/* **ok, it seems we filled the SFR units by using the latest available (2019) SFR unit growth rates;
**need a chart to show the YoY growth rates over time, for US and some markets (how?);
data cj_sfr; set rentUnits_rescale(where=(not missing(indexCode) && (sfr^=. or SFR_g^=.))); 
by indexCode year;  retain sfr_cj; if first.indexCode then sfr_cj=.; if sfr_cj=. && sfr>0 
then sfr_cj=sfr; else sfr_cj=sfr_cj*SFR_g; if sfr_cj^=.; keep indexCode year sfr_cj; proc means; run;

data compare; merge sfrUnit(in=f1) cj_sfr(in=f2); by indexCode year; if f1; 
not_f2=0; if not f2 then not_f2=1; proc means; class not_f2; var sfr_cj sfr; run; 
*/



data HP; set irs.hpi_basefile; indexcode=put(cbsa_Code,$5.); drop cbsa_code; if qtr>0; 
proc means; class qtr; var refi_rate slope; run;

data fundHPI; set irs.asg_hpi_dt(where=(date>200000) keep=indexcode date fundHPI aggregate monthfmt);
qtr = year(monthfmt)*100+qtr(monthfmt); ln_fundHPI = log(fundHPI);
ln_hpi = log(aggregate); if month(monthfmt) in (3,6,9,12);  run;


proc SQL; connect to odbc(DSN='Apollo_3Party'); 
create table baseRent as select * from connection to odbc
(
...
);  disconnect from odbc;quit;	**use median rent for Q1'2013 as base rent;
...
from allrentIdx a join rawRentidx b on a.indexcode=b.indexcode and b.date=201303
join baseRent_med m  on a.indexcode=m.indexcode
join HP c on a.indexcode=c.indexcode and a.qtr=c.qtr 
join HP c1 on a.indexcode=c1.indexcode and  c1.qtr=201301
join allrentIdx e on a.indexcode=e.indexcode and e.qtr=201301
left join cbsaemp em on a.indexcode=em.cbsa_code
and year(a.indexmonth)*100+qtr(a.indexmonth) = year(em.qtr)*100+qtr(em.qtr)
left join sfrUnit sfr on sfr.indexcode=a.indexcode  and year(a.indexmonth) = sfr.year
join geoW w on a.indexcode=w.cbsa

left join fundHPI f on a.indexcode= f.indexcode and a.qtr=f.qtr
where a.qtr>=200001 and a.indexcode in (select indexcode from HP
); ;quit;  proc sort nodup; by indexcode indexmonth; run;

proc sql; create table dup as select * from modelinp0b
group by indexcode, indexmonth having count(1)>1; quit; **empty dataset, good; 
...
data checkTax;set myData.taxhist_stat_2010_2019; by GeographyType GeoCode Year; 
where GeographyType='CBSA'; taxRate=TAX_AMOUNT_Mean/total_value_mean; 
if taxRate<0.005 or taxRate>0.035 then taxRate=TAX_AMOUNT_p50/total_value_p50; 
if taxRate>0.06 or taxRate<0.005 then taxRate=.; if GeoCode^=. && Year^=. && geoCode>0;
call symput(compress("cbsaTax"||trim(geocode)||trim(year)), taxRate); run;

/*proc means data=modelinp0b; class qtr; var LnRentg medRent baseRent refi_rate baseHP slope 
Inc inc_mean us_unemp unemp incG Avginc_G hpi_sa housing hpg_season geoW; where abs(LnRentg)>.1; run; */


data modelinp1; set modelinp0b; by indexcode  indexmonth; retain  Rentidx1 ;  
if first.indexcode  then Rentidx1=1; else Rentidx1=Rentidx1*(exp(LnRentg)); 
ln_Rentidx=log(Rentidx1); month = intck('qtr','01JAN2000'D, indexmonth)+1;

if abs(LnRentg)>.1 and qtr<202001 then LnRentg=.; if abs(lnRentg)>.25 then lnrentg=.;

rentyield=log(medRent*12/hpi_sa_medhp); unemp_l1=lag(unemp);   ln_inc_l1=lag(ln_inc);
hpg_season_l1=lag(hpg_season); medHP_l1 = lag(hpi_sa_medhp);   rentyield_l1=lag(rentyield);
medRent_l1 = lag(medRent);   ln_rentIdx_l1 = lag(ln_rentidx);  unemp_perc = unemp*100;

if first.indexcode then do; ln_inc_l1=.; hpg_season_l1=.; medHP_l1=.;unemp_l1=.; rentyield_l1=.;
medRent_l1=.;ln_rentIdx_l1=.; end;   inc_g=ln_inc - ln_inc_l1;  unemp_g=unemp-unemp_l1; 
...
rent_share = medRent_l1*12/inc;  insurerate=0.5;  year=min(year(indexmonth),2018);
...
mort_share = medHP_l1*factor/(Inc/12);    if mort_share>1 or mort_share<0.1 then mort_share=.; 
if rent_share>1 or rent_share<0.1 then rent_share=.; rent2Own=min(2,max(0.5,medRent_l1/(medHP_l1*factor)));
diff_mort_rent = (mort_share-rent_share);   if rent_share=. or mort_share=. then rent2Own=.;   
ln_rent2Own = log(rent2Own); *diff_mort_rent_l1 = lag(diff_mort_rent); 	*if first.cbsa then diff_mort_rent_l1=.;
inc_gini=log(Inc_mean/inc);		ln_SFDhh = log(sfdhousehold);		run; **200802 has wrong rent2Own ratio;

proc means data=modelinp1 p1 p99 p50 mean; *class qtr; var rent_share mort_share diff_mort_rent 
rent2Own inc_gini ln_SFDhh; where qtr>=201001 and qtr<=202001; run;
...
proc sql; create table modelinp2 as select distinct a.*, h.m_mortshare, h.m_rentShare
from modelinp1 a join histmean h on a.indexcode= h.indexcode order by indexcode, indexmonth; quit;


data modelinp2; set modelinp2; afford_HP= log(Inc*m_mortshare/12/factor)-log(baseHP);
afford_Rent=log(Inc*m_rentshare/12)-log(baseRent);  		proc means; var afford_rent afford_HP; 
class qtr; class inst_group; class pSFR_group; class rentocc_group;  *pSFR_group has some pattern; run;

data order; set parm_fund( keep =_type_);  rn+1;  run; **what's this table for? Not constructed;

 * find outlier market: stage 2&3 by msrMarket;	proc sort data=modelinp2; by msrmarket;  
proc reg data=modelinp2(where=(qtr>201100 and qtr<202103)) outest=parm_test_outlier adjrsq noprint tableout ;  
model lnRentg =rentg_l1 rentg_l2 rentg_l3 rentg_l4 rentg_l5 /selection=stepwise sle=0.05;
by msrMarket; weight geoW; output out=insample_outlier r=r_stage2 p=p_rentg; run; quit;

proc reg data=insample_outlier(where=(qtr>201100 and qtr<202103)) outest=parm_test_outlier1 adjrsq noprint tableout ;
model r_stage2 = hpg_season unemp_g hpg_season_l1-hpg_season_l4 rentyield_l1 chgslope0_1 /selection=stepwise sle=0.05;
by msrMarket; weight geoW; output out=insample_outlier2 r=r_Rentg p=p_rentg2; run; quit;
**Hmm, why do we use these 2-step regression to identify outlier markets? especially the 2nd step...; 

proc sort data=parm_test_outlier; by msrmarket _model_ _type_;  proc sort data=parm_test_outlier1; by msrmarket _model_ _type_;
data parm_test_outlier2; merge parm_test_outlier(rename=(intercept=int1)) parm_test_outlier1(rename=(intercept=int2)); 
by msrmarket _model_ _type_; if int1=. then int1=0; if int2=. then int2=0; intercept=int1+int2; run;

proc score data=insample_outlier2  score=parm_test_outlier2 out=fc_outlier type=parms; by msrmarket;
var rentg_l1 rentg_l2 rentg_l3 rentg_l4 rentg_l5 hpg_season unemp_g hpg_season_l1-hpg_season_l4 rentyield_l1 chgslope0_1 ; run;

proc sort data=fc_outlier; by indexcode qtr; run;	PROC REG data=fc_outlier (where=(qtr>201100 and qtr<202103)) 
outest=reg_act_outlier adjrsq tableout noprint;  by indexcode;   model lnRentg =model1;  run; quit; 

data outlier; set reg_act_outlier; *if indexcode not in &amherstmarket.; if _type_='PARMS'; drop _type_;
proc sort; by descending model1; run;	proc univariate; var model1; run;

proc sql; create table outlier as select c.Name, a.* from outlier a
join thirdp.cbsa_dt c on a.indexcode=c.cbsa where a.model1<0.35 or a.model1>1.3; quit;
**I would go with 10th and 90th percentile, so 0.35 and 1.3, not 0.5 and 1.5; 
...
**I think we don't need stage-2 regression above. Also, we need to drop those markets for which R-Square is so low 
or even negative, plus those markets for which the beta's (AR model and fundamental line) are too high or low;

*also based on the quick look I did above, I think we need to group the markets by pSFR_group;

proc sort data=modelinp2 nodup; by pSFR_group msrMarket; 
proc means data=modelinp2(where=(qtr>201100 and qtr<202103)); class msrMarket pSFR_group; 
weight geoW; var ln_Rentidx inc_gini month afford_rent rent2own ln_inc afford_HP; run;

proc reg data=modelinp2(where=(qtr>201100 and qtr<202103))  outest=parm_fund_outlier adjrsq noprint tableout; 
model ln_Rentidx =afford_rent ln_inc; ** ln_sfdhh; by pSFR_group; weight geoW; *by msrMarket;
output out=fund_outlier r=r_fund p=p_fund; run; quit;  proc means; var r_fund p_fund ln_Rentidx; run; 
data p_fund_outlier; set parm_fund_outlier; if _TYPE_ in ('PARMS', 'T');  proc sort nodup; by _TYPE_ pSFR_group; run;

proc sort data=fund_outlier nodup; by pSFR_group indexcode;  proc reg data=fund_outlier outest=beta_fund_outlier
adjrsq noprint tableout; model ln_Rentidx =p_fund; by pSFR_group indexcode; weight geoW; run; 
data true_outlier; set beta_fund_outlier; where (_TYPE_='T' and p_fund<5) or (_TYPE_='PARMS' and _ADJRSQ_<0.5); 
**only one market with T<5, indexCode=21340, plus another indexCode =10740 with R-Square=0.55; run;
data beta_outlier; set beta_fund_outlier; where ((p_fund<0.5 && _ADJRSQ_<0.8) or (p_fund>2 && _ADJRSQ_<0.95))
&& _TYPE_='PARMS'; run; **added another indexCode=36420 for p_fund<0.5 and another 16700,36100,39460,37100 for p_fund>2; 


proc reg data=modelinp2 outest=parm_test_outlier0 adjrsq noprint tableout ; by pSFR_group; weight geoW; 
model ln_Rentidx =rentg_l1 rentg_l2 rentg_l3 rentg_l4 rentg_l5/selection=stepwise sle=0.05;
where qtr>201100 && qtr<202103 && indexCode not in ('21340','10740','36420','16700','36100','39460','37100'); 
output out=insample_outlier0 r=r_Rentg p=p_rentg; run; quit;
**Hmm, why do we use these 2-step regression to identify outlier markets? especially the 2nd step...; 

proc sort data=insample_outlier0 nodup; by pSFR_group indexcode;  PROC REG data=insample_outlier0 
outest=reg_act_outlier adjrsq  tableout noprint; by pSFR_group indexcode;  model lnRentg=p_rentg; quit;  
data outlier; set reg_act_outlier; by pSFR_group indexcode;  if _type_='PARMS'; drop _TYPE_;   
if (p_rentg <0.5 && _ADJRSQ_<0.05) or (p_rentg>1.25 && _ADJRSQ_<0.25) or p_rentg>1.5; **54 markets to be excluded; 
proc sort; by descending _ADJRSQ_ p_rentg; run;	proc univariate; var p_rentg _ADJRSQ_; run;




 %macro isBlank(param); %sysevalf(%superq(param)=,boolean) %mend ;


%let version=1; %let geoW=geoW; %let byMarket=by msrmarket ; %let fundV = month; %let byV =msrmarket ; 
%let stage3V = inc_g; %put &bymarket.; 		data t; t=%isBlank(&bymarket.); 	run;


%macro tryModel(version=, geoW=, byMarket=, fundV =, byV=, stage3V=, where=);

proc sort data=modelinp3; by  &byV. indexcode qtr; run;
proc reg data=modelinp3(where=(qtr>201100 and qtr<202103 &where.)) outest=parm_fund_v&version. adjrsq noprint tableout ; 
model ln_Rentidx = &fundV. /selection=stepwise sle=0.001; **ln_SFR ln_laborforce ln_laborForce ln_emp; 
&bymarket.; weight &geoW.;  output out=is_fund_v&version. r=r_fund p=rentidx_fund; run; quit;

** 2nd Stage: AR(5);  proc sort data=is_fund_v&version.; by &byV. indexcode qtr; run;

data is_fund_v&version.; set is_fund_v&version.(where=(rentidx_fund ne .)) ; *intercept=1; by &byV. indexcode qtr;
l1_fundidx=lag(rentidx_fund); if first.indexcode then l1_fundidx=.;  rhpits = ln_rentidx_l1-l1_fundidx; 
rhpits_l1 = lag(rhpits); 	if indexcode ne lag(indexcode) then rhpits_l1=.; 	run;

proc reg data=is_fund_v&version.(where=(qtr>201100 and qtr<202103)) outest=parm_stage2_v&version. adjrsq tableout noprint; 
model lnRentg = rhpits rentg_l1 rentg_l2 rentg_l3 rentg_l4 rentg_l5/selection=stepwise sle=0.05;
&bymarket.; weight &geoW.; 	output out=is_stage2_v&version. r=r_stage2 ; run; quit;

** 3rd stage: HPA and other Var;
proc reg data=is_stage2_v&version.(where=(qtr>201100 and qtr<202103)) outest=parm_stage3_v&version. adjrsq tableout noprint; 
...
&bymarket.; weight &geoW.;  output out=resid_SFrent_v&version. r=r_SFRent p=P_rentg; run; quit; 

*** Combined R-Square;

proc means data=resid_SFrent_v&version. noprint nway; &byMarket.; 
var lnRentg; weight &geoW.; output out=tp_mean mean=avg_rentidx; run;

%if %isBlank(&bymarket.)=1 %then %do;
proc sql; create table test as select distinct a.*, b.avg_rentidx 
from resid_SFrent_v&version. a join tp_mean b on 1=1; quit;
data test; set test; r_tot = (lnRentg - avg_rentidx)**2; r_res = r_SFRent**2; run;

%end;  %else %do;
data test; merge resid_SFrent_v&version.(in=f1) tp_mean(keep=&byV. avg_rentidx);&bymarket.; if f1;
r_tot = (lnRentg - avg_rentidx)**2; r_res = r_SFRent**2; run;	%end;

proc means data=test(where=(r_res ne .)) noprint nway; &bymarket; weight &geoW.; var r_tot r_res; 
output out=tp_sum sum=; run;   data tp_sum; set tp_sum; r2 = 1-r_res/r_tot; run;
...
data parm_final_v&version.; merge parm_fund_v&version.(in=f0 rename=(intercept=fund_int _rsq_ = fund_r2 
_adjRsq_ = fund_adjr2) drop=_depvar_ _rmse_ ln_rentidx _in_ _p_ _edf_ ) parm_stage2_v&version.(in=f1 
rename=(intercept=stage2_int _rsq_ = stage2_r2 _adjRsq_ = stage2_adjr2) keep = _model_ _type_  rhpits  
rentg_l1 rentg_l2 rentg_l3 rentg_l4 rentg_l5 intercept &byV. _rsq_ _adjRsq_) parm_stage3_v&version.
(in=f2 rename=(intercept=stage3_int _rsq_ = stage3_r2 _adjRsq_ = stage3_adjr2) keep = _model_ _type_  hpg_season 
unemp_g  hpg_season_l1-hpg_season_l4 rentyield_l1  chgslope0_1 &stage3V. intercept &byV. _rsq_ _adjRsq_);
by &byV. _MODEL_ _type_;	tp_1= rentg_l1; tp_2 = rentg_l2; tp_3=rentg_l3; tp_4=rentg_l4; tp_5=rentg_l5;
if tp_1=. then tp_1=0; 	if tp_2=. then tp_2=0; if tp_3=. then tp_3=0; if tp_4=. then tp_4=0; if tp_5=. then tp_5=0; 	
if _type_='PARMS' then sum_ar = tp_1 + tp_2 + tp_3 + tp_4 + tp_5;  drop tp_1 tp_2 tp_3 tp_4 tp_5;	run;

proc sql; create table parm_final_v&version. as select a.*, b.rn
from parm_final_v&version. a left join order b on a._type_ =b._type_; quit; proc sort ; by &byV. _MODEL_ rn; run;  
data parm_final_v&version.;set parm_final_v&version.; drop rn; run;

%if %isBlank(&bymarket.)=1 %then %do;
proc sql; create table parm_final_v&version. as select distinct a.*, b.r2 as totalR2
from parm_final_v&version. a join tp_sum b on 1=1; quit;

data  parm_final_v&version.; set  parm_final_v&version.;
length version $10.; version=put("&version.", $10.); run;
%end;	%else %do;
data parm_final_v&version.; merge parm_final_v&version.(in=f1) tp_sum(keep=&byV. r2 rename=(r2=totalR2)); &byMarket.; if f1;
length version $10.; version=put("&version.", $10.);  run;   %end;

data finalParm; set parm_final_v&version.(where=(_type_='PARMS')); intercept= stage2_int + stage3_int; 
drop fund_int &fundV. fund_r2 fund_adjr2 stage2_r2 stage2_adjr2 stage3_r2 stage3_adjr2 stage2_int stage3_int TOTALr2; run;
...
proc sort data=fc_v&version. ; by indexcode qtr; run;	PROC REG data=fc_v&version. (where=(qtr>201100 and qtr<202103)) outest=reg_act_v&version. 
adjrsq tableout noprint;  by indexcode;   model lnRentg = model1;  run; quit; 

DATA reg_act_v&version.; set reg_act_v&version.(keep=indexcode intercept model1 _edf_ _rsq_ _adjRsq_ _type_);
length version $10.; version=put("&version.", $10.);  run;		%mend;
...
data summary_all;retain idea ver msrMarket pSFR_group pSFR_group2 inst_group inst_group2 _type_ fund_int month ln_inc 
inc_gini afford_rent rent2Own afford_HP ln_SFDhh  fund_r2 fund_adjr2 stage2_int  rhpits  rentg_l1 rentg_l2 rentg_l3
rentg_l4 rentg_l5 sum_ar stage2_r2 stage2_adjr2 stage3_int hpg_season unemp_g  hpg_season_l1-hpg_season_l4 rentyield_l1  
chgslope0_1 inc_g stage3_r2 stage3_adjr2 totalR2; set parm_final_v:;  length idea ver $5.; idea =substr(version,1,1);
ver = substr(version,2,1); if ver='' then ver='0a'; drop _model_ version;  if psfr_group2=1 or inst_group2=1 then delete;
if _type_ ne 'PARMS' then totalR2=.; if pSFR_group2 ne . then pSFR_group2 = psfr_group2-1; 
if inst_group2 ne . then inst_group2 = inst_group2-1; run;

...
%global pp_file_date;
data _null_; date=nwkdom(2, 6, &sysmonth, &sysyear); pp_file_date=year(date)*10000+month(date)*100+day(date);
call symput ("pp_file_date",strip(pp_file_date)); run;


%global firstdate fhfastateurl  fhfamsaurl cbsa100list statelist nondisclosure deletestate nopricetier deletemsa countylist;
option compress=yes;	 
%let firstdate=200001;
%let fhfastateurl=http://www.fhfa.gov/DataTools/Downloads/Documents/HPI/HPI_EXP_state.txt;
%let fhfamsaurl=http://www.fhfa.gov/DataTools/Downloads/Documents/HPI/HPI_EXP_metro.txt;
%let cbsa100list=( 10420	10580	10740	10900	11244	12060	12420	12540	12580	12940	13820	14454	14860	15380	15764	15804	16700	16740	16974	17140	17460	17820	17900	18140	19124	19380	19740	19804	21340	22744	23104	23420	23844	24340	24660	24860	25540	26420	26900	27260	28140	28940	29404	29820	30780	31084	31140	32580	32820	33124	33340	33460	33874	34980	35004	35084	35300	35380	35614	35840	36084	36420	36540	36740	37100	37964	38060	38300	38900	39300	39580	40060	40140	40380	40900	41180	41620	41700	41740	41884	41940	41980	42644	43524	44140	44700	45060	45104	45300	46060	46140	46520	47260	47664	47894	48424	48620	48864	49180	49340);

proc sql noprint; select distinct case when cbsa_div is null then cbsa else cbsa_div end into: cbsa100list separated by ',' from wlres.zipcodesdotcom_dt
where case when cbsa_div is null then cbsa else cbsa_div end is not null; quit;
%let cbsa100list=(&cbsa100list);
%put &cbsa100list;

%let statelist =AL AR  AZ CA CO CT DC DE FL GA HI IA IL KY MA MD MI MN NC NE NH NJ NV NY OH OK OR PA RI SC SD TN VA VT WA WI WV 
AK IN KS MT NM ND UT WY MO ID LA ME MS TX);
*%let nondisclosure = ('AK' 'IN' 'KS' 'MT' 'NM' 'ND' 'UT' 'WY' 'MO' 'ID' 'LA' 'ME' 'MS' 'TX');
%let nondisclosure = ('');
%let deletestate=('AA','AE','AP','US');
*%let deletestate=('AA','AE','AP','WV','SD','DC','US');
*%let nopricetier=(10420, 10740, 12940, 13820, 15380, 17140, 17460, 17900, 19380, 23844, 24340, 24660, 28140, 
31140, 32820, 41180, 41620, 44140, 46520, 49180,18140,23420);
%let nopricetier=();

%let deletemsa=(10740,12420,12940,19124,21340,23104,23844,26420,26900,28140,32580,35380,41180,41620,41700,48620);
...
data cpi; month=.; run;
%add_fredRent(inp=cpi,sm_url=https://fred.stlouisfed.org/data/CPIAUCSL.txt,sm_var=cpi,sm_firstobs=16);run;
data cpi; set cpi; if cpi ne .;run;
...
data testbed.cremacr_mapcbsa; set mapcbsa;run;
*/

data mapCBSA; set testbed.cremacr_mapcbsa; run;
...
%global fcqtrStart;
data _null_; set rawrentidx3; by indexcode date; if last.date; 
...
data USHPA; set irs.asg_hpi_dt; where indexcode='US'; US_ProdHPI=aggregate; if mod(date,100) in (3,6,9,12); indexmonth=input(put(date*100+1,8.),YYMMDD10.);format indexmonth  MMDDYYD10.; 

...
, case when p_SFR<=p2.sfr_33 then 1 when p_SFR<=p2.sfr_67 then 2 else 3 end as pSFR_group2
...
*/
proc export data=geoW outfile="E:\Output\Rent Forecast\weight by cbsa 1207.csv" dbms=csv replace; run;
...
afford_Rent = log(Inc*m_rentshare/12) - log(baseRent);
 run;
 proc  means ; class pSFR_group;*inst_group; var afford_rent; run;
...
data outlier1; set ar_bygeo; where sum_ar<0.4 or sum_ar>1.5 or _RSQ_<0.1; keep indexcode; proc sort nodup; by indexcode; run;
...
proc export data=parm_test_outlier outfile="E:\Output\Rent Forecast\simple AR5 parm.csv" dbms=csv replace; run;
...
data modelinp3; set modelinp2; where indexcode not in (&outlier1.); run;
